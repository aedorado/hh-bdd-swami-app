...
class Adapter {
  static MediaItem audioToMediaItem(Audio? audio) {
    return MediaItem(id: audio?.id ?? '', album: 'BDD Swami Vani', title: audio?.name ?? '', extras: {
      'url': audio?.url,
  ...
  static MediaItem firebaseAudioSnapshotToMediaItem(QueryDocumentSnapshot<Object?> doc) {
    return MediaItem(id: doc['id'] ?? '', album: 'BDD Swami Vani', title: doc['name'] ?? '', extras: {
      'url': doc['url'] ?? '',
  ...
  static Audio firebaseAudioSnapshotToAudio(QueryDocumentSnapshot<Object?> doc) {
    return Audio(id: doc['id'] ?? '', name: doc['name'] ?? '', url: doc['url'] ?? '', thumbnailUrl: doc['thumbnailUrl'] ?? '');
  }
